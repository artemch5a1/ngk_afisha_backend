...
public class GetAllInvitationByAuthorHandler
    : IRequestHandler<GetAllInvitationByAuthorQuery, Result<List<Invitation>>>
{
    ...
    public async Task<Result<List<Invitation>>> Handle(
        GetAllInvitationByAuthorQuery request,
        CancellationToken cancellationToken
    )
    {
        try
        {
            List<Invitation> invitations = await _invitationService.GetAllInvitationByAuthor(
                request.AuthorId,
                request.Contract,
                cancellationToken
            );
