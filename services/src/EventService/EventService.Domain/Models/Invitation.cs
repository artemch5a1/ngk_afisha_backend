...
public class Invitation
{
        ...
        DateTime deadLine)
    {
        ...
        return new Invitation(invitationId, eventId, roleId, shortDescription, description, requiredMember, deadLine);
    }

    internal void Update(string shortDescription, 
        string description, 
        int requiredMember,
        DateTime deadLine, 
        int? roleId = null)
    {
        ...
        if (_members.Count > requiredMember)
            throw new DomainException("Новое количество требуемых участников меньше, чем количество уже набраных");
    ...
    {
        Member? member = _members.FirstOrDefault(x => x.StudentId == studentId && x.Status != MemberStatus.Accepted);
    ...
    {
        var member = _members.FirstOrDefault(x => x.StudentId == studentId)
