...
public class EventRepository : IEventRepository
{
    ...
    public async Task<List<Event>> GetAll(ISpecification<Event>? specification, PaginationContract? contract = null, CancellationToken cancellationToken = default)
    {
        try
        {
            Expression<Func<EventEntity, bool>> spec = specification is null ? 
                entity => true  : _efSpecificationMapper.ResolveEfSpecification<Event, EventEntity>(specification);
    ...
    public async Task<List<Event>> GetAll(PaginationContract? contract = null, CancellationToken cancellationToken = default)
    {
        try
        {
            List<EventEntity> result = contract is null ? 
    ...
    public async Task<Event?> GetById(Guid id, CancellationToken cancellationToken = default)
    {
        try
        {
            EventEntity? result = await _db.Events
                .Include(x => x.Genre)
                .Include(x => x.Type)
                .Include(x => x.Location)
                .Include(x => x.Invitations)
    ...
    public async Task<Event> Create(Event model, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            EntityEntry<EventEntity> createdEvent = await _db.Events.AddAsync(eventEntity, cancellationToken);
    ...
    public async Task<bool> UpdateEventAggregate(Event updatedDomainEvent, CancellationToken ct = default)
    {
        try
        {
    ...
    private async Task UpdateInvitations(EventEntity existingEvent, Event updatedDomainEvent)
    {
        ...
        var updatedInvitations = updatedDomainEvent.Invitations
            .Select(_invitationMapper.ToEntity)
            .ToList();
        ...
        {
            InvitationEntity? existingInvitation = existingEvent.Invitations
                .FirstOrDefault(i => i.InvitationId == updatedInvitation.InvitationId);
    ...
    private void RemoveDeletedInvitations(EventEntity existingEvent, List<InvitationEntity> updatedInvitations)
    {
    ...
    private async Task UpdateMembers(InvitationEntity existingInvitation, InvitationEntity updatedInvitation)
    {
        ...
        {
            MemberEntity? existingMember = existingInvitation.Members
                .FirstOrDefault(m =>
