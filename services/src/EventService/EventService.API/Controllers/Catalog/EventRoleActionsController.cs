...
public class EventRoleActionsController : ControllerBase
{
    ...
    {
        Result<List<EventRole>> result = 
            await _mediator.Send(new GetAllEventRoleQuery(null), cancellationToken);
    ...
    public async Task<ActionResult<EventRoleDto>> GetEventRoleById(
        ...
        CancellationToken cancellationToken)
    {
        Result<EventRole> result = 
            await _mediator.Send(new GetEventRoleByIdQuery(roleId), cancellationToken);
    ...
    public async Task<ActionResult<EventRoleDto>> CreateEventRole([FromBody] CreateEventRoleDto dto, CancellationToken cancellationToken)
    {
        Result<EventRole> createdLocation = 
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdateEventRole([FromBody] UpdateEventRoleDto dto, CancellationToken cancellationToken)
    {
        Result<int> result = 
            await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> DeleteEventType(int roleId, CancellationToken cancellationToken)
    {
        Result<int> resultDelete = 
            await _mediator.Send(new DeleteEventRoleCommand(roleId), cancellationToken);
