...
public class LocationActionsController : ControllerBase
{
    ...
    {
        Result<List<Location>> result = await _mediator.Send(
            new GetAllLocationQuery(null),
            cancellationToken
        );
    ...
    {
        Result<Location> result = await _mediator.Send(
    ...
    {
        Result<Location> createdLocation = await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdateLocation(
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> DeleteLocation(
        int locationId,
        CancellationToken cancellationToken
    )
    {
        Result<int> resultDelete = await _mediator.Send(
            new DeleteLocationCommand(locationId),
            cancellationToken
        );
