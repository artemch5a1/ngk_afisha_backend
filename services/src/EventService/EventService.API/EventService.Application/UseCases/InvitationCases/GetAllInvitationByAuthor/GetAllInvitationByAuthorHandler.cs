...
public class GetAllInvitationByAuthorHandler : IRequestHandler<GetAllInvitationByAuthorQuery, Result<List<Invitation>>>
{
    private readonly IInvitationService _invitationService;

    private readonly ILogger<GetAllInvitationByAuthorHandler> _logger;

    private readonly IStorageService _storageService;

    private readonly EventSetting _eventSetting;
    
    public GetAllInvitationByAuthorHandler(
        IInvitationService invitationService, 
        ILogger<GetAllInvitationByAuthorHandler> logger,
    ...
    public async Task<Result<List<Invitation>>> Handle(GetAllInvitationByAuthorQuery request, CancellationToken cancellationToken)
    {
        try
        {
            List<Invitation> invitations =
                await _invitationService.GetAllInvitationByAuthor(request.AuthorId, request.Contract, cancellationToken);
