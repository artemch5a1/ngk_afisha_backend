...
public class GenresActionsController : ControllerBase
{
    ...
    [HttpGet("GetAllGenre")]
    public async Task<ActionResult<List<GenreDto>>> GetAllGenre(
        CancellationToken cancellationToken)
    {
        Result<List<Genre>> result = 
            await _mediator.Send(new GetAllGenreQuery(null), cancellationToken);
    ...
    {
        Result<Genre> result = 
            await _mediator.Send(new GetGenreByIdQuery(genreId), cancellationToken);
    ...
    public async Task<ActionResult<GenreDto>> CreateGenre([FromBody] CreateGenreDto dto, CancellationToken cancellationToken)
    {
        Result<Genre> createdLocation = 
    ...
    [HttpPut("UpdateGenre")]
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdateGenre([FromBody] UpdateGenreDto dto, CancellationToken cancellationToken)
    {
        Result<int> result = 
            await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> DeleteGenre(int genreId, CancellationToken cancellationToken)
    {
