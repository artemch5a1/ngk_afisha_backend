...
public class UserActionsController : ControllerBase
{
    ...
    {
        Result<List<User>> result = await _mediator.Send(new GetAllUsersQuery(), ct);
    ...
    {
        Result<User> result = await _mediator.Send(new GetUserByIdQuery(userId), ct);
    ...
    public async Task<ActionResult<UserDto>> GetCurrentUser(CancellationToken ct)
    {
        ...
        Result<User> result = await _mediator.Send(new GetUserByIdQuery(userId.Value), ct);
    ...
    public async Task<ActionResult<Guid>> UpdateUserInfo(UpdateUserDto dto, CancellationToken ct)
    {
        ...
        Result<Guid> result = await _mediator.Send(dto.ToCommand(userId.Value), ct);
    ...
    [Authorize]
    public async Task<ActionResult<Guid>> UpdateStudentProfile(
    ...
    [Authorize]
    public async Task<ActionResult<Guid>> UpdatePublisherProfile(
        UpdatePublisherProfileDto dto,
        CancellationToken ct
    )
    {
        Result<Guid> userId = User.ExtractGuid();
