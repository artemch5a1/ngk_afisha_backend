...
public class PostActionsController : ControllerBase
{
    ...
    {
        Result<Post> result = await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [HttpPut("UpdatePost")]
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdatePost(
    ...
    {
        Result<int> result = await _mediator.Send(new DeletePostCommand(postId), cancellationToken);
    ...
    {
        Result<Post> result = await _mediator.Send(new GetPostByIdQuery(postId), cancellationToken);
    ...
    {
        Result<List<Post>> result = await _mediator.Send(new GetAllPostQuery(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.PublisherOrAdmin)]
    public async Task<ActionResult<List<PostDto>>> GetAllPostByDepartment(
        int departmentId,
        CancellationToken cancellationToken
    )
    {
        Result<List<Post>> result = await _mediator.Send(
            new GetAllPostByDepartmentQuery(departmentId),
            cancellationToken
        );
