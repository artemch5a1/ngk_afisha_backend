...
public class DepartmentActionsController : ControllerBase
{
    ...
    {
        Result<Department> result = await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdateDepartment(
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> DeleteDepartment(
        int departmentId,
        CancellationToken cancellationToken
    )
    {
    ...
    {
        Result<Department> result = await _mediator.Send(
    ...
    [Authorize(Policy = PolicyNames.PublisherOrAdmin)]
    public async Task<ActionResult<List<DepartmentDto>>> GetAllSpecialty(
        CancellationToken cancellationToken
    )
    {
        Result<List<Department>> result = await _mediator.Send(
