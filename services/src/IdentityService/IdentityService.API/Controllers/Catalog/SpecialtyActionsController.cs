...
public class SpecialtyActionsController
{
    ...
    {
        Result<Specialty> result = await _mediator.Send(dto.ToCommand(), cancellationToken);
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> UpdateSpecialty(
    ...
    [Authorize(Policy = PolicyNames.AdminOnly)]
    public async Task<ActionResult<int>> DeleteSpecialty(
        int specialtyId,
        CancellationToken cancellationToken
    )
    {
        Result<int> result = await _mediator.Send(
            new DeleteSpecialtyCommand(specialtyId),
            cancellationToken
        );
    ...
    {
        Result<Specialty> result = await _mediator.Send(
    ...
    public async Task<ActionResult<List<SpecialtyDto>>> GetAllSpecialty(
        CancellationToken cancellationToken
    )
    {
        Result<List<Specialty>> result = await _mediator.Send(
            new GetAllSpecialtyQuery(),
            cancellationToken
        );
