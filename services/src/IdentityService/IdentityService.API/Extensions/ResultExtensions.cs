...
public static class ResultExtensions
{
    public static ActionResult<T> ToActionResult<T>(this Result<T> result)
    {
        ...
        return new ObjectResult(result.ErrorMessages) { StatusCode = (int)result.ErrorType };
    }

    public static ActionResult<TNewModel> ToActionResult<TNewModel, T>(
        this Result<T> result,
        Func<T, TNewModel> mapper
    )
    {
        if (result.IsSuccess)
        {
