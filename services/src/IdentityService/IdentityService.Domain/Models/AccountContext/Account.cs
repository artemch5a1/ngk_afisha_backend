...
{
    private Account(Guid accountId, string email, string passwordHash, DateTime createdDate, Role role)
    {
        AccountId = accountId;
    ...
    public static Account CreateStudentAccount(string email, string password, IPasswordHasher passwordHasher)
    {
    ...
    /// <exception cref="DomainException">Выбрасывается при пустом или некорректном email.</exception>
    public static Account CreatePublisherAccount(string email, string password, IPasswordHasher passwordHasher)
    {
    ...
    internal static Account Restore(Guid accountId, string email, string passwordHash, DateTime createdDate, Role role)
    {
    ...
    private static Account CreateAccount(Guid id, string email, string password, Role role, IPasswordHasher passwordHasher)
    {
        if (string.IsNullOrEmpty(email))
    ...
    {
        string pattern = @"^[a-zA-Z0-9]([\w\.-]*[a-zA-Z0-9])?@([a-zA-Z0-9]([\w-]*[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$";
