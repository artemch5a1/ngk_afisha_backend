...
public class AccountServiceTests
{
    ...
    public async Task LoginAsync_ShouldReturnAccount_WhenPasswordCorrect()
    {
        ...
        _accountRepoMock
            .Setup(r => r.FindOnlyUsersByEmail("user@test.com", It.IsAny<CancellationToken>()))
    ...
    public async Task LoginAsync_ShouldReturnNull_WhenAccountNotFound()
    {
        // Arrange
        _accountRepoMock
            .Setup(r => r.FindOnlyUsersByEmail("missing@test.com", It.IsAny<CancellationToken>()))
    ...
    {
        // Arrange
        var admin = Account.CreatePublisherAccount(
            "admin@test.com",
            "adminpass",
            _passwordHasherMock.Object
        );
        _accountRepoMock
            .Setup(r => r.FindAdminByEmail("admin@test.com", It.IsAny<CancellationToken>()))
            .ReturnsAsync(admin);
    ...
    public async Task CreateStudentAccount_ShouldCreateAndSaveAccount()
    {
        // Arrange
        _accountRepoMock
            .Setup(r => r.Create(It.IsAny<Account>(), It.IsAny<CancellationToken>()))
        ...
        _accountRepoMock.Verify(
            r => r.Create(It.IsAny<Account>(), It.IsAny<CancellationToken>()),
            Times.Once
        );
    }
    ...
    public async Task CreateStudentAccount_ShouldThrow_WhenEmailInvalid()
    {
        ...
        // Assert
        await act.Should().ThrowAsync<DomainException>().WithMessage("*Некорректный email*");
    }

    [Fact]
    public async Task CreateStudentAccount_ShouldThrow_WhenEmailEmpty()
    {
        await Assert.ThrowsAsync<DomainException>(() => _service.CreateStudentAccount("", "1234"));
    }
    ...
    public async Task ChangeAccountPassword_ShouldUpdate_WhenOldPasswordCorrect()
    {
        ...
        _accountRepoMock
            .Setup(r => r.FindAsync(account.AccountId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(account);
        _accountRepoMock
            .Setup(r => r.Update(account, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task ChangeAccountPassword_ShouldThrow_WhenAccountNotFound()
    {
        ...
        // Act
        Func<Task> act = async () =>
            await _service.ChangeAccountPassword(Guid.NewGuid(), "1234", "5678");

        // Assert
        await act.Should().ThrowAsync<NotFoundException>().WithMessage("*Аккаунт*");
    }
    ...
    public async Task ChangeAccountPassword_ShouldThrow_WhenOldPasswordIncorrect()
    {
        ...
        // Act
        Func<Task> act = async () =>
            await _service.ChangeAccountPassword(account.AccountId, "wrong", "5678");

        // Assert
        await act.Should().ThrowAsync<DomainException>().WithMessage("*Неверный пароль*");
    }
    ...
    public async Task GetAccountById_ShouldReturnAccount_WhenExists()
    {
        ...
        _accountRepoMock
            .Setup(r => r.GetById(account.AccountId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(account);
    ...
    public async Task GetAllAccounts_ShouldReturnList()
    {
        ...
        _accountRepoMock.Setup(r => r.GetAll(It.IsAny<CancellationToken>())).ReturnsAsync(list);
