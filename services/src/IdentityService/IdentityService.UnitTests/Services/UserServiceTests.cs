...
public class UserServiceTests
{
    ...
    public async Task GetUserById_ShouldReturnUser_WhenExists()
    {
        ...
        _userRepoMock
            .Setup(r => r.GetById(user.UserId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(user);
    ...
    public async Task CreateStudent_ShouldCreateAndSaveUser()
    {
        ...
        var result = await _service.CreateStudent(
            userId,
            "Иванов",
            "Иван",
            null,
        ...
        _userRepoMock.Verify(
            r => r.Create(It.IsAny<User>(), It.IsAny<CancellationToken>()),
            Times.Once
        );
    }
    ...
    public async Task CreatePublisher_ShouldCreateAndSaveUser()
    {
        ...
        var result = await _service.CreatePublisher(
            userId,
            "Петров",
            "Пётр",
            "Петрович",
    ...
    public async Task CreateStudent_ShouldThrow_WhenTooYoung()
    {
        ...
        Func<Task> act = async () =>
            await _service.CreateStudent(userId, "Иванов", "Иван", null, tooYoung, 1);

        await act.Should().ThrowAsync<DomainException>().WithMessage("*младше 14 лет*");
    }
    ...
    public async Task UpdateUserInfo_ShouldUpdateFields_WhenUserExists()
    {
        ...
        _userRepoMock
            .Setup(r => r.FindAsync(user.UserId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(user);
        _userRepoMock.Setup(r => r.Update(user, It.IsAny<CancellationToken>())).ReturnsAsync(true);

        var result = await _service.UpdateUserInfo(
            user.UserId,
            "Сидоров",
            "Сидор",
            null,
    ...
    public async Task UpdateUserInfo_ShouldThrow_WhenUserNotFound()
    {
        ...
        Func<Task> act = async () =>
            await _service.UpdateUserInfo(Guid.NewGuid(), "А", "Б", null, new DateOnly(2000, 1, 1));

        await act.Should().ThrowAsync<NotFoundException>().WithMessage("*Пользователь*");
    }
    ...
    public async Task UpdateStudentProfile_ShouldUpdate_WhenExists()
    {
        ...
        _userRepoMock
            ...
            .ReturnsAsync(user);
        _userRepoMock
            .Setup(r => r.UpdateStudentProfile(user, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task UpdatePublisherProfile_ShouldUpdate_WhenExists()
    {
        ...
        _userRepoMock
            ...
            .ReturnsAsync(user);
        _userRepoMock
            .Setup(r => r.UpdatePublisherProfile(user, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
