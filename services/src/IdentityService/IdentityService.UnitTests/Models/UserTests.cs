...
public class UserTests
{
    [Fact]
        ...
        {
            // Act
            Action act = () => User.Restore(Guid.Empty, "Иванов", "Иван", null, new DateOnly(2000, 1, 1));
        ...
        {
            // Arrange
            var user = User.CreatePublisher(Guid.NewGuid(), "Петров", "Пётр", "Иванович", DateOnly.FromDateTime(DateTime.UtcNow.AddYears(-40)), 2);

            // Act
            user.UpdateFields("Сидоров", "Сидор", null, DateOnly.FromDateTime(DateTime.UtcNow.AddYears(-25)));
        ...
        {
            // Arrange
            var user = User.CreateStudent(Guid.NewGuid(), "Иванов", "Иван", "Иванович", DateOnly.FromDateTime(DateTime.UtcNow.AddYears(-22)), 101);
        ...
        {
            // Arrange
            var user = User.CreatePublisher(Guid.NewGuid(), "Иванов", "Иван", null, DateOnly.FromDateTime(DateTime.UtcNow.AddYears(-22)), 1);
            ...
            // Assert
            act.Should().Throw<DomainException>()
                .WithMessage("*нет студенческого профиля*");
        }
        ...
        {
            // Arrange
            var user = User.CreateStudent(Guid.NewGuid(), "Иванов", "Иван", null, DateOnly.FromDateTime(DateTime.UtcNow.AddYears(-22)), 101);
            ...
            // Assert
            act.Should().Throw<DomainException>()
                .WithMessage("*нет профиля публикатора*");
        }
