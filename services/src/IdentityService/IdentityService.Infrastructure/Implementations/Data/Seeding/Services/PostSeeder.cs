...
public class PostSeeder : ISeedService
{
    ...
    public PostSeeder(
        ...
        IOptions<PostSeedOptions> options)
    {
        _db = db;
    ...
    public async Task SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            DepartmentEntity department = await _db
                .Departments
                .FirstAsync(x => x.Title == postSeed.DepartmentName,
                    cancellationToken);
            
            await _db.Posts.AddAsync(new PostEntity()
            {
