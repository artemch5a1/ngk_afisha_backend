...
public class GetSpecialtyByIdHandler : IRequestHandler<GetSpecialtyByIdQuery, Result<Specialty>>
{
    ...
    public GetSpecialtyByIdHandler(ISpecialtyService specialtyService, ILogger<GetSpecialtyByIdHandler> logger)
    {
        _specialtyService = specialtyService;
        _logger = logger;
    }

    public async Task<Result<Specialty>> Handle(GetSpecialtyByIdQuery request, CancellationToken cancellationToken)
    {
        try
        {
            Specialty? result = await _specialtyService.GetSpecialtyById(request.SpecialtyId, cancellationToken);
