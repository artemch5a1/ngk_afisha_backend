...
public class UpdateUserHandler : IRequestHandler<UpdateUserCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(UpdateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(ex, "Доменная ошибка при обновлении пользователя: {UserId}", request.UserId);
            return Result<Guid>.Failure(ex);
        }
        catch (DatabaseException ex)
        {
            _logger.LogWarning(ex, "Ошибка базы данных при обновлении пользователя: {UserId}", request.UserId);
            return Result<Guid>.Failure(ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Произошла ошибка при обновлении пользователя: {UserId}", request.UserId);
